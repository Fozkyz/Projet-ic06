...
public class PlayerController : MonoBehaviour
{
	...
	private void RunCollisionChecks()
	{
		...
		{
			// Only trigger when first touching
			_coyoteUsable = true;
	...
	private void OnDrawGizmos()
    {
		...
		Vector3 move = new Vector3(_currentHorizontalSpeed, _currentVerticalSpeed) * Time.deltaTime;
		Gizmos.DrawWireCube(transform.position + _characterBounds.center, _characterBounds.size);
    }
	...
	private void CalculateWalk()
	{
		if (InputFrame.Horizontal != 0)
		{
			...
			_currentHorizontalSpeed += InputFrame.Horizontal * _acceleration * Time.deltaTime;

			// Clamped by max frame movement
			_currentHorizontalSpeed = Mathf.Clamp(_currentHorizontalSpeed, -_moveClamp, _moveClamp);

			// Apply bonus at the apex of a jump
			var apexBonus = Mathf.Sign(InputFrame.Horizontal) * _apexBonus * _apexPoint;
	...
	private void CalculateJump()
	{
		...
		if (InputFrame.JumpDown && CanUseCoyote || HasBufferedJump)
		{
		...
		if (!_colDown && InputFrame.JumpUp && !_endedJumpEarly && Velocity.y > 0)
		{
	...
	[Header("Move")]
	[SerializeField, Tooltip("Raising this value increases collision accuracy at the cost of performance.")] private int _freeColliderIterations = 10;
	...
	private void MoveCharacter()
	{
		...
		// Check furthest movement. If nothing hits, move and don't do extra checks
